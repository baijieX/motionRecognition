...
void
GlobalCoupler::updateRXMutualInductances(){
	int i = GlobalCoupler::partialZMatrix->nRow()-1;
	...
	if(!GlobalCoupler::coilContainer[i].isUpdated()){
		...
		for(int j=0; j<i; j++){
			if(!GlobalCoupler::coilContainer[j].isUpdated()){
				calculateMutualInductance(i,j);
				GlobalCoupler::allTheSame=false;//if at least one coil is modified, all
					//current calculations must be performed again next time.
			}
	...
	//and set the coils updated
		...
		for(int j=0; j<i; j++){
			(*GlobalCoupler::partialZMatrix)(j+1,i+1) = (*GlobalCoupler::partialZMatrix)(i+1,j+1);
		}
}
